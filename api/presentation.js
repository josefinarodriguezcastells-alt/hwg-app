  ...
  try {
    if (req.method === 'GET') {
      const resp = await fetch(
        `${SUPABASE_URL}/rest/v1/candidate_presentations?token=eq.${encodeURIComponent(token)}&select=*`,
        { headers: baseHeaders }
      );
    ...
    if (req.method === 'PATCH') {
      ...
      // el link — el único campo que este endpoint público puede tocar.
      const resp = await fetch(
        `${SUPABASE_URL}/rest/v1/candidate_presentations?token=eq.${encodeURIComponent(token)}`,
        {
          method: 'PATCH',
